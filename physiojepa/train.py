...
# %% auto 0
__all__ = ['PatchTSJEPALightning', 'PatchTFTSingleOutcomeLightning', 'LinearBaselineSupervised']
...
from .models import PatchTSJEPA, LinearPatchTimeClassifier
...
# %% ../nbs/06_train.ipynb 6
class LinearBaselineSupervised(pl.LightningModule):
    def __init__(self, 
                 learning_rate, # desired learning rate, initial learning rate in if one_cycle_scheduler
                 train_loader_size, # the training data size (for one_cycle_scheduler=True)
                 linear_patch_kwargs,
                 metrics={}, # name:function for metrics to log
        ...
        self.gamma = gamma
        self.optimizer_type = optimizer_type
        self.model = LinearPatchTimeClassifier(**linear_patch_kwargs)
        self.save_hyperparameters()
    ...
    def training_step(self, batch, batch_idx):
        ...
        focal_loss =  FocalLoss(weight=self.class_weights.to(x.device) if self.class_weights is not None else None, gamma=self.gamma, ignore_index=self.y_padding_mask)
        ce_loss_val = ce_loss(x,y) 
        focal_loss_val = focal_loss(x,y)
        loss = ce_loss_val if self.loss_fxn == 'CrossEntropy' else focal_loss_val
        self.log('train_loss', loss, prog_bar=True, on_step=True, on_epoch=True, sync_dist=True)
        self.log('train_ce_loss' if self.loss_fxn != 'CrossEntropy' else 'train_focal_loss', ce_loss_val if self.loss_fxn != 'CrossEntropy' else focal_loss_val, prog_bar=True, on_step=True, on_epoch=True, sync_dist=True)
        if x.dim() == 3:
            x = x[:,1,:].squeeze(-1)
        if y.dim() == 2:
            y = y.squeeze(-1)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay) if self.optimizer_type.lower() == 'adamw' else\
                     torch.optim.Adam(self.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay)
        if self.scheduler_type.lower() == 'onecycle':
            scheduler = OneCycleLR(optimizer, max_lr=self.max_lr, epochs=self.epochs, steps_per_epoch=(self.train_loader_size))
            lr_scheduler = {'scheduler': scheduler, 'interval': 'step'}
            return {'optimizer': optimizer, 'lr_scheduler': lr_scheduler}
        elif self.scheduler_type.lower() == 'cosineannealingwarmrestarts':
            scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=self.epochs//10, T_mult=2, eta_min=1e-8)
